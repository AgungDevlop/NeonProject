...
function initializeDiagnosisChart() {
    ...
    diagnosisChart = new Chart(ctx, {
        type: 'bar',
        data: { labels: [], datasets: [{ label: '% CPU Usage', data: [], backgroundColor: 'rgba(16, 185, 129, 0.5)', borderColor: 'rgba(16, 185, 129, 1)', borderWidth: 1 }] },
        options: {
            indexAxis: 'y', responsive: true, maintainAspectRatio: false,
            scales: { x: { beginAtZero: true, max: 100, ticks: { color: '#9ca3af' }, grid: { color: 'rgba(156, 163, 175, 0.1)' } }, y: { ticks: { color: '#9ca3af' }, grid: { display: false } } },
            plugins: { legend: { display: false }, tooltip: { backgroundColor: '#030712', titleColor: '#34d399', bodyColor: '#f3f4f6' } }
        }
    });
}
...
function parseTopOutput(output) {
    try {
        ...
        return processLines.map(line => {
            ...
            return { name: parts[parts.length - 1], cpu: parseFloat(parts[8]) || 0 };
...
function updateDiagnosis(processes) {
    ...
    if (highUsageProcess) {
        alertDiv.innerHTML = `<i class="fas fa-exclamation-triangle mr-2"></i><strong>High CPU Usage:</strong> <code>${highUsageProcess.name}</code> at <strong>${highUsageProcess.cpu.toFixed(1)}% CPU</strong> may cause slowdowns.`;
        alertDiv.className = 'diagnosis-alert';
    } else {
        alertDiv.className = 'hidden';
    }
    diagnosisChart.data.labels = processes.map(p => p.name);
    diagnosisChart.data.datasets[0].data = processes.map(p => p.cpu);
...
async function runDiagnosisCycle() {
    ...
    } catch (e) { console.error('Diagnosis cycle failed:', e); stopDiagnosis(); }
}
...
function renderTweakComponents() {
    ...
    createRadioOptions('jit-profile-options', [
        { id: 'speed', name: 'Speed Profile', value: 'jit_speed_profile' },
    ], 'jit_profile');
    
    createRadioOptions('game-driver-options', [
        { id: 'performance', name: 'Force Performance Driver', value: 'force_game_driver_performance' },
    ], 'game_driver');

    createRadioOptions('network-profile-options', [
        { id: 'gaming', name: 'Gaming (Low Latency)', value: 'network_gaming' },
        { id: 'streaming', name: 'Streaming (High Throughput)', value: 'network_streaming' }
    ], 'network_profile');
}
